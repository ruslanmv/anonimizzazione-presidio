...
def compute(source_df):
    ...
    def anonymize_custom(text):    
        def anonymize_id(text):
            id_pattern = r'[A-Z]\d+'  # Assuming IDs start with a capital letter followed by digits
            anonymized_text = re.sub(id_pattern, '<ANONYMIZED>', text)
            return anonymized_text
        
        def anonymize_ndg_number(text):
            ndg_pattern = r'ndg\s+(\d+)'  # Assuming NDG numbers are preceded by "ndg" and followed by digits
    ...
    def anonymize_description_multi(description):
        ...
        anonymized_results = anonymizer.anonymize(text=description, analyzer_results=analyzer_results, operators=operators)
        return anonymized_results.text
    
    # Register the UDF
    anonymize_description_multi_udf = udf(anonymize_description_multi, StringType())
